...
public class GalleryController {
	...
	public String update(GalleryVO vo, MultipartFile file, HttpSession session, String attach, Model model) {
	...
	public String delete(int id, HttpSession session, Model model) {
		...
		// 해당 갤러리글 정보를 DB에서 삭제한 후 목록화면으로 연결
	...
	public void download(int id, HttpSession session, HttpServletResponse response) {
		...
		map.put("id", Integer.toString(id));
		map.put("loginInfo_member_id", member.getMember_id());
		// 해당 갤러리글의 첨부파일 정보를 DB에서 조회해와 해당 파일을 서버로부터 다운로드 함.
		GalleryVO gallery = service.gallery_detail(map);
	...
	public String detail(int id, Model model, HttpSession session) {
		...
		// 선택한(id) 갤러리 정보를 DB에서 조회해 와 상세화면 출력
...
//			vo.setWriter(member.getId());
		...
		return "redirect:list.gal"; // 리턴 시 갤러리 목록 화면으로 이동 처리
	}
