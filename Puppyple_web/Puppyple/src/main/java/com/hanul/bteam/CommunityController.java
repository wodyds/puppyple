...
public class CommunityController {
	...
	public String comment_update(@RequestBody CommunityCommentVO vo) {
		return service.community_comment_update(vo) > 0 ? "댓글이 수정되었습니다" : "댓글 수정에 실패하였습니다";
	}
	...
	// 자유게시판 글에 대한 댓글 목록조회 요청
	@RequestMapping("/community/comment/list/{pid}")
	...
	public String update(CommunityVO vo, MultipartFile file, HttpSession session, String attach, Model model) {
		...
		CommunityVO community = service.community_detail(map);
		String uuid = session.getServletContext().getRealPath("resources") + "/" + community.getBoard_filepath();
		...
		if (!file.isEmpty()) {
			...
			// 서버에 파일이 있는지 파악
			if (community.getBoard_filename() != null) {
			...
			if (attach.isEmpty()) { // 첨부된 파일명이 없을 때
				if (community.getBoard_filepath() != null) { // 원래 첨부된 파일이 있었다면
					File f = new File(uuid);
					if (f.exists())
				...
				vo.setBoard_filename(community.getBoard_filename());
				vo.setBoard_filepath(community.getBoard_filepath());
			}
		}
	...
	public String delete(int id, HttpSession session, Model model) {
		...
		// 해당 자유게시판글 정보를 DB에서 삭제한 후 목록화면으로 연결
		service.community_delete(id);
		model.addAttribute("uri", "list.com");
		model.addAttribute("page", page);
//						return "community/redirect";
		return "redirect:list.com";
	}

	// 자유게시판글의 첨부 파일을 다운로드 요청
	...
	public void download(int id, HttpSession session, HttpServletResponse response) {
		...
		// 해당 자유게시판글의 첨부파일 정보를 DB에서 조회해와 해당 파일을 서버로부터 다운로드 함.
		CommunityVO community = service.community_detail(map);
		common.fileDownload(community.getBoard_filename(), community.getBoard_filepath(), session, response);
	...
	public String detail(int id, HttpSession session, Model model) {
		...
		// 선택한(id) 자유게시판 정보를 DB에서 조회해 와 상세화면 출력
		model.addAttribute("vo", service.community_detail(map));
		model.addAttribute("crlf", "\r\n");
		model.addAttribute("page", page);
	...
	public String insert(CommunityVO vo, HttpSession session, MultipartFile file) {
...
//			
		...
		return "redirect:list.com"; // 리턴 시 자유게시판 목록 화면으로 이동 처리
	}

	// 자유게시판글 수정 화면 요청
	@RequestMapping("/modify.com")
	public String modify(int id, HttpSession session, Model model) {
	...
	public String list(HttpSession session, String search, String keyword, String flag,
		...
		session.setAttribute("category", "com");
		// DB에서 자유게시판글 목록을 조회해와 목록화면에 출력
//			model.addAttribute("list", service.community_list());
