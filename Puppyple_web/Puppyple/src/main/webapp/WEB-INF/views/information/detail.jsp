		...
		<div class="board_content_wrap">
			...
			<c:if test="${ not empty vo.board_filename }">
				<div class="board_filename_c">${vo.board_filename }<a
						href="download.info?id=${vo.id } " style="margin-left: 9px;"><i
			...
			<div class="button comment_div">
				...
				<div class="list_btn2">
					<c:if
						...
						<a class="btn-fill"
							onclick=" $('form').attr('action', 'modify.info');
						$('form').submit()">글
							수정</a>
						<a class="btn-fill" id="btn-delete"
							onclick=" if(confirm('정말 삭제하시겠습니까?')) { href='delete.info?id=${vo.id }'} ">글
							삭제</a>
					</c:if>
				</div>
...
<script type="text/javascript">
	...
	function comment_regist(){
		...
		$.ajax ({
			// 경로 형태로 url 지정
			url : "information/comment/regist"
		...
		function comment_list() {
			$.ajax({
				url: "information/comment/list/${vo.id}"
//				, data : {pid:${vo.id}}
				, success : function(response) {
