...
<body>
	<div id="new_content_wrap">
		...
		<div class="modify_title_div">
			<h1 class="modify_title">갤러리 글 수정</h1>
		</div>
		<form method="post" enctype="multipart/form-data" action="update.gal">
			...
			<table>
				<tr>
					...
					<td><input type="text" name='board_title' title='제목'
						class="new_title_wrap" value="${vo.board_title }" class='chk'
						autofocus /></td>
				</tr>
				<tr>
					<th>내용</th>
					<td><textarea name='board_content' class="new_content_wrap"
							class='chk' title='내용'>${vo.board_content}</textarea></td>
				</tr>
				<tr>
					<th>첨부파일</th>
					<td>
						<div class="board_file-name">
							...
							<label> <a><img src='imgs/select.png'
									class='file-img' /></a> <input type="file" name='file'
								id='attach-file' />
							</label> <span id='file-name'>${vo.board_filename }</span>
							<!-- 첨부 파일이 있었는데 삭제하는 경우 -->
							<a id='delete-file'
								style='display: ${empty vo.board_filename ? "none" : "inline"}'>
		...
		<div class='button'>
			...
			<a class='btn-empty' href='detail.gal?id=${vo.id}'>취소</a>
		</div>
