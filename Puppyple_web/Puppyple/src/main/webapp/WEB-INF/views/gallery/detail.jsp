			...
			<div class="button comment_div">
				...
				<div class="list_btn2">
					<c:if
						...
						<a class="btn-fill"
							onclick=" $('form').attr('action', 'modify.gal'); $('form').submit()">글
							수정</a>
						<a class="btn-fill" id="btn-delete"
							onclick=" if(confirm('정말 삭제하시겠습니까?')) { href='delete.gal?id=${vo.id }'} ">글
							삭제</a>
					</c:if>
				</div>
...
<script type="text/javascript">
	...
	function comment_regist(){
		...
		function comment_list() {
			$.ajax({
				url: "gallery/comment/list/${vo.id}"
//				, data : {pid:${vo.id}}
				, success : function(response) {
