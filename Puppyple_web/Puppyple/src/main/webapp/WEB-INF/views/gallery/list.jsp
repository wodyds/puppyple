	...
	<div id="content_wrap">
		...
		<div id="community_menubar">
			<ul>
				...
				<li><a href="list.gal" class="on_listpage">갤 러 리</a></li>
				<li><a href="list.tra">중고 거래</a></li>
			</ul>
			<div class="title_div">
				<h1 class="title">
					갤러리<span class="small_title"><span class="small_title_l">|</span>
						gallery</span>
				</h1>
			</div>
		</div>

		<form action="list.gal" method="post">
			...
			<div id="list_wrap">
				<ul id="list_grid_wrap">
					...
					<c:forEach items="${page.list }" var="vo">
						<li>
							<%-- <div>${vo.no}</div> --%> <c:if
								test="${ not empty vo.board_filename }">
								<a style="background-image : url(${vo.board_filepath });"
									id="preview" href='detail.gal?id=${vo.id }'></a>
								<!-- 미리보기 이미지가 나오는곳 -->
							</c:if> <c:if test="${empty vo.board_filename }">
								<a id="preview_not" href='detail.gal?id=${vo.id }'></a>
								<!-- 파일이미지가 없을 경우 기본 이미지가 나오는곳 -->
							</c:if>

							<div class="list_grid_title">
								<a href='detail.gal?id=${vo.id }'>${vo.board_title }</a>
		...
		<c:if test="${! empty loginInfo }">
			<div class="writing">
				<ul class="writing_wrap">
					<li><a href='new.gal' class="btn-fill">글쓰기</a></li>
				</ul>
			</div>
		</c:if>
