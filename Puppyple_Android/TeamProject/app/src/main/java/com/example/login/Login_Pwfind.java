...
import com.example.ATask.PwSelect;
...
public class Login_Pwfind extends AppCompatActivity {
    ...
    ImageView pwfind_back;
    TextView textView;
    android.widget.Button Button,emailCodeButton;
    EditText idText,emailText,emailCodeText;
    ...
    public void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        pwfind_back = findViewById(R.id.pwfind_back);
        pwfind_back.setOnClickListener(new View.OnClickListener() {
            @Override
        ...
        emailCodeButton.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                if(emailCodeText.getText().toString().equals(GmailCode)){
                    ...
                    PwSelect pwSelect = new PwSelect(member_id,member_email,dtos);
                    try {
                        pwSelect.execute().get();
                    } catch (ExecutionException e) {
                        e.printStackTrace();
                    } catch (InterruptedException e) {
                        e.printStackTrace();
                    }
                    AlertDialog.Builder builder = new AlertDialog.Builder(Login_Pwfind.this);
                    builder.setTitle("아이디 찾기 결과");
                    if(dtos.size() != 0){
                        builder.setMessage("아이디 : "+dtos.get(0).getMember_id()+"\n"+"비밀번호 : "+dtos.get(0).getMember_pw());
                    }
    ...
    class MailTread extends Thread{
        public void run(){
            ...
            try {
                // 제목, 본문내용, 받는사람
                gMailSender.sendMail("퍼피플 비밀번호 찾기","퍼피플 비밀번호 찾기입니다 \n인증번호 입력란에 "+GmailCode+" 를 입력해주세요. \n인증번호는 영소문자,숫자 조합입니다. \n찾으신 비밀번호는 보안을 위해 수정하는것을 추천합니다.",emailText.getText().toString());
            } catch (Exception e) {
                e.printStackTrace();
            }
        }
