...
import com.example.ATask.NoticeModify;
import com.example.DTO.BoardDTO;
import com.example.PUPPYPLE.R;

import java.util.concurrent.ExecutionException;

public class Notice_Modify extends AppCompatActivity implements AdapterView.OnItemSelectedListener {
    ...
    protected void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        reg_btn.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                ...
                NoticeModify noticeModify = new
                        NoticeModify(id, flag, member_id, board_title, board_content);

                try {
                    state = noticeModify.execute().get().trim();
                ...
                if (state.equals("1")) {
                    Toast.makeText(Notice_Modify.this,
                            "공지글 수정 완료", Toast.LENGTH_SHORT).show();

                    finish();
                } else {
                    Toast.makeText(Notice_Modify.this,
                            "공지글 수정에 실패 하였습니다. 다시 시도해주세요.", Toast.LENGTH_SHORT).show();
                }
